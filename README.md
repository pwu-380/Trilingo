...
**Mad Libs** — Fill-in-the-blank sentence completion.
...
**Tune In** — Listening comprehension: hear a word, pick the Chinese.
- A word's audio plays automatically when the round starts
- Tap the play button to replay the audio anytime
- Pick the correct Chinese word from 4 options
- Wrong answers are disabled; you keep trying, but the round is marked incorrect on first wrong attempt
- Correct answers reveal the pinyin and English translation
- **Hint** — Toggle to show pinyin for the correct answer
...
**Scramble Harder** — Like Scrambler, but harder: unscramble either the Chinese or the English, with decoy words mixed in.
- The prompt shows one language; you assemble the translation from word tiles
- Decoy words from other sentences are mixed into the tile pool — you must translate, not just unscramble
- Tap tiles to place them in order; tap placed tiles to remove; Clear resets
- Check your answer when all slots are filled; wrong answers shake and let you retry (round marked incorrect)
- **Pinyin hint** — Available when unscrambling Chinese
- Correct answers show both the full Chinese (with pinyin) and the English
- Requires at least 20 generated sentences at your HSK level to unlock

**Session summary** — After all rounds, see your score, percentage, and a message. Perfect scores get a congratulations image.

**Rate limit handling** — If the AI rate limit is hit during Mad Libs, the game falls back to cached questions and shows an info notification. The chatbot also shows a clear message if rate limited.
